...
excludedClassNames = [className.strip() for className in open('excludedClasses.config').readlines()]

# hit the site and store the html, converted to snapshot
cachedContent = {}
for site in sites:
    response = urlopen(Request(site, headers={'User-Agent': 'Mozilla/5.0'}))
    cachedContent[site] = buildSnapShotFromHtml(response.read(), excludedElementTypes, excludedClassNames)
